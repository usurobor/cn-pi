...
class EffectBridgeTests(unittest.TestCase):
    ...
    def test_only_explicit_outbox_is_an_effect_source(self) -> None:
    ...
    def test_effect_frames_recover_independently(self) -> None:
        ...
        document = (
            f"<<<CN-PI-EFFECT-BEGIN id=pi-effect-test-001>>>\n{one}\n"
            "<<<CN-PI-EFFECT-END id=pi-effect-test-001>>>\n"
            f"<<<CN-PI-EFFECT-BEGIN id=pi-effect-test-002>>>\n{two}\n"
            "<<<CN-PI-EFFECT-END id=pi-effect-test-002>>>\n"
            "<<<CN-PI-EFFECT-BEGIN id=pi-effect-test-003>>>\n{"
        )
        records = bridge.extract_effect_records(document)
        self.assertEqual([record[0] for record in records], [
            "pi-effect-test-001", "pi-effect-test-002", "pi-effect-test-003"
        ])
    ...
    def test_incomplete_frame_is_pending_not_terminal_or_executed(self) -> None:
        ...
        with mock.patch.object(bridge, "execute_effect") as execute:
            result = bridge.sync_effect_requests(
                self.route,
                args,
                [("drive-file-id", payload, "https://example.invalid", "rclone:Pi — Outbox — TSC Sigma.docx@now")],
            )
        execute.assert_not_called()
        self.assertEqual(result["requests_seen"][0]["status"], "pending")
        self.assertEqual(result["requests_seen"][0]["error"]["code"], "incomplete_frame")

    def test_small_resource_surface_is_closed(self) -> None:
        comment = self.request(
            method="DELETE", path="/issues/2/comments/123", body=None
        )
        validated = bridge.validate_effect_request(comment["id"], comment, self.route)
        self.assertEqual(validated["repo"], "usurobor/tsc")
        document = self.request(
            method="PUT",
            path="/documents/docs/note.md",
            body={"message": "docs: update note", "content": "# Note\n"},
        )
        self.assertEqual(
            bridge.validate_effect_request(document["id"], document, self.route)["path"],
            "/documents/docs/note.md",
        )
        with self.assertRaisesRegex(bridge.SyncError, "outside issues"):
            bridge.validate_effect_request(
                "pi-effect-test-001",
                self.request(method="POST", path="/pulls", body={}),
                self.route,
            )
        with self.assertRaisesRegex(bridge.SyncError, "documentation extensions"):
            bridge.validate_effect_request(
                "pi-effect-test-001",
                self.request(
                    method="PUT",
                    path="/documents/src/main.py",
                    body={"message": "bad", "content": "print('no')"},
                ),
                self.route,
            )

    def test_issue_collection_filters_pull_requests(self) -> None:
        request = bridge.validate_effect_request(
            "pi-effect-test-001", self.request(path="/issues"), self.route
        )
        with (
            mock.patch.object(bridge, "github_token", return_value="token"),
            mock.patch.object(
                bridge,
                "github_rest",
                return_value=(200, [{"number": 1}, {"number": 2, "pull_request": {}}]),
            ),
        ):
            status, result = bridge.execute_effect(request)
        self.assertEqual(status, 200)
        self.assertEqual(result, [{"number": 1}])

    def test_pull_request_is_rejected_as_an_issue(self) -> None:
        with mock.patch.object(
            bridge, "github_rest", return_value=(200, {"number": 3, "pull_request": {}})
        ):
            with self.assertRaisesRegex(bridge.SyncError, "pull requests are not"):
                bridge.ordinary_issue("token", "usurobor/tsc", 3)

    def test_document_result_decodes_text_and_rejects_code(self) -> None:
        content = base64.b64encode(b"# Note\n").decode("ascii")
        result = bridge.document_result({
            "type": "file", "name": "note.md", "encoding": "base64", "content": content
        })
        self.assertEqual(result["content"], "# Note\n")
        self.assertEqual(result["encoding"], "utf-8")
        with self.assertRaisesRegex(bridge.SyncError, "not a textual document"):
            bridge.document_result({
                "type": "file", "name": "main.py", "encoding": "base64", "content": ""
            })

    def test_ledger_replay_does_not_repeat_an_effect_and_collision_is_visible(self) -> None:
        raw = json.dumps(self.request(), separators=(",", ":")).encode()
        digest = bridge.hashlib.sha256(raw).hexdigest()
        with tempfile.TemporaryDirectory() as directory:
            connection = bridge.effect_database(Path(directory) / "effects.sqlite3")
            with mock.patch.object(
                bridge, "execute_effect", return_value=(200, {"number": 2})
            ) as execute:
                first, _ = bridge.retain_effect_result(
                    connection,
                    self.route,
                    "pi-effect-test-001",
                    digest,
                ...
                second, _ = bridge.retain_effect_result(
                    connection,
                    self.route,
                    "pi-effect-test-001",
                    digest,
            ...
            collision, _ = bridge.retain_effect_result(
                connection,
                self.route,
                "pi-effect-test-001",
                "0" * 64,
        ...
        self.assertEqual(collision["error"]["code"], "id_collision")

    def test_interrupted_write_becomes_uncertain_without_replay(self) -> None:
        raw = json.dumps(self.request(method="PATCH", body={"state": "closed"}), separators=(",", ":")).encode()
        digest = bridge.hashlib.sha256(raw).hexdigest()
        with tempfile.TemporaryDirectory() as directory:
            connection = bridge.effect_database(Path(directory) / "effects.sqlite3")
            ...
            connection.execute(
                ...
                ("tsc", "pi-effect-test-001", digest, raw, "executing", b"{}", now, now),
            )
            connection.commit()
            with mock.patch.object(bridge, "execute_effect") as execute:
                result, _ = bridge.retain_effect_result(
                    ...
                    digest,
                    raw,
                    self.request(method="PATCH", body={"state": "closed"}),
                    None,
                    dry_run=False,
                )
            connection.close()
